...
public class StatisticsDaoBean {
    ...
    public List<Object[]> findTop10Recipies() {
        return entityManager.createNamedQuery("Recipe.findTop10Recipies")
            .setMaxResults(10).getResultList();

    }
    public List<Object[]> getCategoryRank() {
        return entityManager.createNativeQuery("select category.name , count(categories) "
            + "as quantity from RecipeStatistics_categories JOIN category WHERE "
            + "categories = category.id group by categories ORDER BY quantity DESC").getResultList();
    }
